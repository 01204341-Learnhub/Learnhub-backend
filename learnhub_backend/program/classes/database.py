...
# ASSIGNMENTS
def edit_assignment(class_id: str, assignment_id: str, patch_body_: PatchAssignmentRequestModel):
    try:
        ...
        if patch_body["due_time"] is not None:
            update_body_edit["$set"]["due_time"] = datetime.fromtimestamp(patch_body["due_time"])
        if patch_body["status"] is not None:
            update_body_edit["$set"]["status"] = patch_body["status"] # TODO:make status do something
            ...
            for i in range(len(patch_body["attachments"])):
                ...
                if patch_body["attachments"][i]["op"] == "add":
                    ...
                    array_filter.append(
                        ...
                        {
                            f"elem{i}.src": str(
                                patch_body["attachments"][i]["old_src"]
                            )
                        }
                    )
